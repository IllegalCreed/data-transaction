...
const { data } = defineProps<{
  data: INewsItem[]
  loading: boolean
  columnList: ITableColumn<INewsItem>[]
  propLabelMap: IPropLabelMap<INewsItem>
}>()

const selectedIds = ref<string[]>([])

const rowKey = (row: INewsItem) => String(row.id)
...
const deleteAll = () => {
  emit('delete', selectedIds.value, '选中资讯')
}
const changeAllStatus = (newStatus: ActiveStatus) => {
  const filteredNews = selectedIds.value
  ...
  const filteredIds = filteredNews.map((news) => (news ? String(news.id) : ''))
  const newsTitles = filteredNews.map((news) => (news ? news.title : '')).join(', ')

  emit('changeStatus', filteredIds, newsTitles, newStatus)
}
</script>
