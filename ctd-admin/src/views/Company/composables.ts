...
export const useDeleteCompanies = (refresh: () => void) => {
  const { deleteCompanies: deleteCompaniesAction } = useCompanyStore()
  ...
  const { doDelAction } = useDelete(
    ...
    async () => {
      await deleteCompaniesAction(delIds.value)
      refresh()
    }
  )
...
export const useChangeCompaniesStatus = (refresh: () => void) => {
  const { changeCompaniesStatus: changeCompaniesStatusAction } = useCompanyStore()
  ...
  const { doChangeAction } = useChangeStatus(
    ...
    async () => {
      ...
      await changeCompaniesStatusAction(changeIds.value, changeStatus.value)
      refresh()
    }
  )
...
export const useSearchCompany = (refresh: () => void) => {
