    ...
    >
      ...
      <div flex flex-col>
        ...
        <div class="panel">
          ...
          <el-select clearable v-model="statusToString" placeholder="选择用户状态">
            <el-option
...
const status = defineModel<UserStatus | null>('status', { default: null })
const statusToString = computed<string | undefined>({
  ...
  set(newValue) {
    if (!newValue) {
      ...
      status.value = newValue as UserStatus
    }
  }
})
...
import { UserStatus, userStatusOptions } from '@/constants/mapData/user'
