    ...
    <div class="content">
      <div flex flex-col flex-1>
        <div class="product-container">
          <div flex flex-col items-start gap-2 flex-1>
            <span class="title">{{ comment.demand.name }}</span>
            <el-tag size="small" class="tag">{{ mappedTypes }}</el-tag>
            <div flex-1></div>
            <div class="price">
              {{ `￥${comment.demand.budget}` }}
            </div>
          </div>
      ...
      <div class="actions-container">
        <el-button class="detail-btn" @click="viewDemand">查看需求</el-button>
...
const mappedTypes = computed(
  () => TRANSACTION_MODE_MAP[comment.demand.transactionType.mode] || '比稿类型',
)
