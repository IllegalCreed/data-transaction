<template>
  <div class="comment-product-root-container">
    <span text-xl font-bold>产品评论</span>
    ...
    <el-skeleton :loading="getProductCommentsActionLoading" animated>
      <template #template>
      ...
      <template #default>
        <div flex flex-col gap-4 v-if="productReviews.length > 0">
...
import CommentProductItem from './CommentProductItem.vue'

import { useCommentStore } from '@/stores/modules/comment'
const commentStore = useCommentStore()
const { productReviews } = storeToRefs(commentStore)
const { getProductComments: getProductCommentsAction } = commentStore
...
const {
  isLoading: getProductCommentsActionLoading,
  execute: executeGetProductCommentsAction,
} = useAsyncState(getProductCommentsAction, undefined, {
  immediate: false,
  throwError: true,
})
...
onMounted(() => {
  try {
    executeGetProductCommentsAction()
  } catch (error: unknown) {
    console.error(error)
  }
})
...
.comment-product-root-container {
