<template>
  <div class="demand-order-detail-review-root-container">
    <span text-lg font-bold>评价信息</span>

    <el-skeleton :loading="getDemandOrderReviewActionLoading" animated>
      <template #template>
...
const { getDemandOrderReview: getDemandOrderReviewAction } = orderStore
...
const {
  state: reviewDetails,
  isLoading: getDemandOrderReviewActionLoading,
  execute: executeGetDemandOrderReviewAction,
} = useAsyncState(() => getDemandOrderReviewAction(orderId), {
  id: 0,
  name: '',
  usefulCount: 0,
  rating: 0,
...
onMounted(() => {
  try {
    executeGetDemandOrderReviewAction()
  } catch (error: unknown) {
    console.error(error)
  }
})
...
.demand-order-detail-review-root-container {
